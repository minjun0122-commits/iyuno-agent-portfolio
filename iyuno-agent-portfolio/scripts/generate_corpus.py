...
publicly documented technical or security topic. Every file's frontmatter lists
the general public standard/body it summarizes plus a retrieval date, so the
agent's citations point to a real, checkable topic rather than to copyrighted
prose. See data/SOURCES.md for the full attribution table.
"""
import json
import pathlib
...
DOCS = [
    ("owasp-top10-overview", "OWASP Top 10 (web application security)", "2026-09-08", """
The OWASP Top 10 is a community-maintained awareness document listing the ten
most critical risks facing web applications. Recent editions emphasize broken
access control as the most frequently observed weakness, followed by
cryptographic failures (previously called "sensitive data exposure") and
injection flaws such as SQL and command injection. The list also tracks
security misconfiguration, vulnerable and outdated components, identification
...
Each category is derived from aggregated incident and testing data contributed
by industry partners, so the ranking shifts slowly release to release rather
than every year. Teams typically use the list as a checklist during design
review and as a taxonomy for classifying findings from automated scanners and
manual penetration tests.
"""),
    ("owasp-llm-top10", "OWASP Top 10 for Large Language Model Applications", "2026-09-08", """
...
mitigate first.
"""),
    ("nist-csf-summary", "NIST Cybersecurity Framework (CSF) 2.0", "2026-09-08", """
The NIST Cybersecurity Framework organizes security practice into six
functions: Govern, Identify, Protect, Detect, Respond, and Recover. Govern
(added in version 2.0) covers how an organization sets risk-management
strategy, roles, and policy. Identify covers asset inventory and risk
assessment. Protect covers access control, awareness training, and data
security. Detect covers continuous monitoring and anomaly detection. Respond
covers incident response planning and communication. Recover covers restoring
capabilities and services after an incident. The framework is deliberately
outcome-based rather than prescriptive, so organizations map their existing
controls to these functions instead of adopting a single fixed control list.
It is widely used as a common vocabulary between technical teams and
executive leadership when discussing security maturity.
"""),
    ("zero-trust-architecture", "NIST SP 800-207 Zero Trust Architecture", "2026-09-08", """
...
trust model.
"""),
    ("api-rate-limiting", "API rate limiting and throttling patterns", "2026-09-08", """
...
"""),
    ("retrieval-augmented-generation", "Retrieval-Augmented Generation (RAG) design patterns", "2026-09-08", """
...
"""),
    ("tool-calling-agents", "Tool calling and function calling in LLM agents", "2026-09-08", """
...
"""),
    ("agent-evaluation-metrics", "Evaluating LLM agent systems", "2026-09-08", """
...
"""),
    ("prompt-injection-defense", "Defenses against prompt injection", "2026-09-08", """
...
"""),
    ("vector-databases-overview", "Vector databases and approximate nearest neighbor search", "2026-09-08", """
...
dedicated ANN index.
"""),
    ("cwe-common-weaknesses", "CWE: Common Weakness Enumeration", "2026-09-08", """
CWE is a community-maintained catalog of the general categories of software
and hardware weaknesses that can lead to vulnerabilities, distinct from CVE,
which tracks specific vulnerability instances in specific products. Frequently
cited entries include CWE-79 (cross-site scripting), CWE-89 (SQL injection),
CWE-20 (improper input validation), CWE-200 (exposure of sensitive
information), and CWE-284 (improper access control). Security teams use CWE
identifiers to classify findings consistently across tools and vendors, and
the annual "CWE Top 25 Most Dangerous Software Weaknesses" list, derived from
aggregated CVE and exploitability data, is commonly used to prioritize secure
coding training and static-analysis rule sets.
"""),
    ("secure-coding-input-validation", "Secure coding: input validation and output encoding", "2026-09-08", """
...
"""),
    ("incident-response-lifecycle", "Incident response lifecycle", "2026-09-08", """
A typical incident response lifecycle has four phases: preparation
(maintaining playbooks, contact lists, and tooling before anything happens),
detection and analysis (confirming an event is a genuine incident and scoping
its impact), containment, eradication, and recovery (isolating affected
systems, removing the root cause, and restoring service), and post-incident
activity (a blameless retrospective that produces concrete follow-up actions).
Clear severity definitions and a designated incident commander role reduce
confusion during a live incident. Communication templates prepared in advance,
for both internal stakeholders and, where required, affected customers or
regulators, shorten response time when minutes matter.
"""),
    ("api-authentication-patterns", "API authentication and authorization patterns", "2026-09-08", """
...
"""),
    ("data-licensing-basics", "Data licensing basics for training and retrieval corpora", "2026-09-08", """
When building a corpus for retrieval or fine-tuning, the license attached to
each source document determines what downstream use is permitted. Public
domain and permissively licensed content (such as material under Creative
Commons Attribution or CC0) generally allows reuse with attribution.
Copyleft licenses (such as GPL-family licenses for code, or CC BY-SA for
text) permit reuse but require derivative works to carry the same license.
"All rights reserved" material requires explicit permission or falls back to
narrow exceptions such as fair use, which is jurisdiction-dependent and fact
specific. Good practice for a project corpus is to record, per document, the
source URL, the license or terms of use, and the date it was retrieved, so
that provenance and permitted use can be audited later.
"""),
    ("latency-cost-tradeoffs", "Latency and cost trade-offs in production LLM systems", "2026-09-08", """
...
accuracy alone.
"""),
    ("logging-and-observability", "Logging and observability for backend systems", "2026-09-08", """
...
"""),
    ("ci-cd-testing-pyramid", "CI/CD and the testing pyramid", "2026-09-08", """
...
"""),
    ("hallucination-and-grounding", "Hallucination and grounding in generative models", "2026-09-08", """
...
"""),
    ("least-privilege-agent-permissions", "Least privilege for autonomous and semi-autonomous agents", "2026-09-08", """
...
for slug, source_topic, retrieved, body in DOCS:
    text = f"""---
title: "{source_topic}"
...
{body.strip()}
"""
    (RAW_DIR / f"{slug}.md").write_text(text, encoding="utf-8")

print(f"Wrote {len(DOCS)} documents to {RAW_DIR}")
